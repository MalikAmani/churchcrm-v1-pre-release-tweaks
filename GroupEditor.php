...
//Set the page title
$sPageTitle = gettext('Ministry Editor');
...
$rsGroupTypes = ListOptionQuery::create()->filterById('3')->find();     // Get Ministry Types for the drop-down
$rsGroupRoleSeed = GroupQuery::create()->filterByRoleListId(['min'=>0], $comparison)->find();     //Ministry Ministry Role List
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title"><?= (($thisGroup->isSundaySchool())?gettext("Special Ministry Settings : Sunday School Type"):gettext('Ministry Settings')) ?></h3>
  </div>
  <div class="box-body">
    <form name="groupEditForm" id="groupEditForm">
      <div class="form-group">
        ...
        <div class="row">
          <div class="col-sm-3">
            <label for="GroupType"><?= gettext('Type of Ministry') ?>:</label>
            <?php
...
// Show Role Clone fields only when adding new group
            if (strlen($iGroupID) < 1) {
                ?>
              <b><?= gettext('Ministry Member Roles') ?>:</b>
        ...
        <div class="row">
          <div class="col-sm-6">
            <label for="UseGroupProps"><?= gettext('Ministry Specific Properties: ') ?></label>

            <?php
            if ($thisGroup->getHasSpecialProps()) {
                echo gettext('Enabled').'<br/>';
                echo '<button type="button" id="disableGroupProps" class="btn btn-danger groupSpecificProperties">'.gettext('Disable Ministry Specific Properties').'</button><br/>';
                echo '<a  class="btn btn-success" href="GroupPropsFormEditor.php?GroupID='.$iGroupID.'">'.gettext('Edit Ministry-Specific Properties Form').' </a>';
            } else {
                echo gettext('Disabled').'<br/>';
                echo '<button type="button" id="enableGroupProps" class="btn btn-danger groupSpecificProperties">'.gettext('Enable Ministry Specific Properties').'</button>&nbsp;';
            }
            ?>
          </div>
        </div>
...
<div class="box">
  <div class="box-header">
    <h3 class="box-title"><?= gettext('Ministry Roles') ?>:</h3>
  </div>
  <div class="box-body">
    <div class="alert alert-info alert-dismissable">
      <i class="fa fa-info"></i>
      <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
      <strong></strong><?= gettext('Ministry role name changes are saved as soon as the box loses focus')?>
    </div>
