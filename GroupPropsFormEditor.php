...
/*******************************************************************************
 *
 *  filename    : MinistryPropsFormEditor.php
 *  last change : 2003-02-09
 *  website     : http://www.churchcrm.io
 *  copyright   : Copyright 2003 Chris Gebhardt (http://www.openserve.org)
 *                Copyright 2013 Michael Wilt
 *
 *  function    : Editor for Ministry-specific properties form
 *
...
// Get the Ministry from the querystring
...
$sPageTitle = gettext('Ministry-Specific Properties Form Editor:').'  '.$grp_Name;
...
if (isset($_POST['SaveChanges'])) {
    ...
    $sSQL = 'SELECT prop_ID, prop_Field, type_ID, prop_Special, prop_PersonDisplay FROM Ministryprop_master WHERE grp_ID = '.$iMinistryID.' ORDER BY prop_ID';
    ...
    if (!$bErrorFlag) {
        for ($iPropID = 1; $iPropID <= $numRows; $iPropID++) {
            ...
            $sSQL = "UPDATE Ministryprop_master
					...
					WHERE `grp_ID` = '".$iMinistryID."' AND `prop_ID` = '".$iPropID."';";
    ...
    if (isset($_POST['AddField'])) {
        ...
        if (strlen($newFieldName) == 0) {
            ...
            $sSQL = 'SELECT prop_Name FROM Ministryprop_master WHERE grp_ID = '.$iMinistryID;
            ...
            if (!$bDuplicateNameError) {
                ...
                $sSQL = 'SELECT prop_ID	FROM Ministryprop_master WHERE grp_ID = '.$iMinistryID;
                $rsPropList = RunQuery($sSQL);
                $newRowNum = mysqli_num_rows($rsPropList) + 1;

                // Find the highest existing field number in the Ministry's table to determine the next free one.
                // This is essentially an auto-incrementing system where deleted numbers are not re-used.
                $tableName = 'Ministryprop_'.$iMinistryID;
                ...
                $sSQL = "INSERT INTO `Ministryprop_master`
							( `grp_ID` , `prop_ID` , `prop_Field` , `prop_Name` , `prop_Description` , `type_ID` , `prop_Special` )
							VALUES ('".$iMinistryID."', '".$newRowNum."', 'c".$newFieldNum."', '".$newFieldName."', '".$newFieldDesc."', '".$newFieldType."', $newSpecial);";
                RunQuery($sSQL);

                // Insert into the Ministry-specific properties table
                $sSQL = 'ALTER TABLE `Ministryprop_'.$iMinistryID.'` ADD `c'.$newFieldNum.'` ';
    ...
    $sSQL = 'SELECT * FROM Ministryprop_master WHERE grp_ID = '.$iMinistryID.' ORDER BY prop_ID';
...
<form method="post" action="MinistryPropsFormEditor.php?MinistryID=<?= $iMinistryID ?>" name="MinistryPropFormEditor">
    ...
    for ($row = 1; $row <= $numRows; $row++) {
		...
		<tr>
			...
			<td class="TextColumn" width="5%" nowrap>
				<?php
                if ($row != 1) {
                    echo "<a href=\"MinistryPropsFormRowOps.php?MinistryID=$iMinistryID&PropID=$row&Field=".$aFieldFields[$row].'&Action=up"><i class="fa fa-arrow-up"></i></a>';
                }
        if ($row < $numRows) {
            echo "<a href=\"MinistryPropsFormRowOps.php?MinistryID=$iMinistryID&PropID=$row&Field=".$aFieldFields[$row].'&Action=down"><i class="fa fa-arrow-down"></i></a>';
        } ?>

				<?= "<a href=\"MinistryPropsFormRowOps.php?MinistryID=$iMinistryID&PropID=$row&Field=$aFieldFields[$row]&Action=delete\"><i class='fa fa-times' ></i></a>"; ?>
			</td>
            ...
            if ($aTypeFields[$row] == 9) {
                ...
                echo '<option value="0" selected>'.gettext('Select a Ministry').'</option>';

                $sSQL = 'SELECT grp_ID,grp_Name FROM Ministry_grp ORDER BY grp_Name';

                $rsMinistryList = RunQuery($sSQL);

                while ($aRow = mysqli_fetch_array($rsMinistryList)) {
                    extract($aRow);
                ...
                if ($aSpecialErrors[$row]) {
                    echo '<span style="color: red;"><BR>'.gettext('You must select a Ministry.').'</span>';
                }
            } elseif ($aTypeFields[$row] == 12) {
                echo "<a href=\"javascript:void(0)\" onClick=\"Newwin=window.open('OptionManager.php?mode=Ministrycustom&ListID=$aSpecialFields[$row]','Newwin','toolbar=no,status=no,width=400,height=500')\">Edit List Options</a>";
            } else {
                echo '&nbsp;';
            } ?></td>
